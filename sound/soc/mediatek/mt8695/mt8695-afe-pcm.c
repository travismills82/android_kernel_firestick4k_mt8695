				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	if (memif->avsync_mode) {
		spin_lock_irqsave(&memif->buf_info_lock, flags);
		buf_read_bef = memif->buf_read % memif->buffer_size;
		...
		ucontrol->value.integer.value[0] =
		    bytes_to_frames(memif->substream->runtime, memif->buf_read);
		ucontrol->value.integer.value[1] =
		    bytes_to_frames(memif->substream->runtime, memif->buf_write);
...
static int mt8695_afe_hdmi_timestamp_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 2;
	return 0;
...
static const struct snd_kcontrol_new mt8695_afe_controls[] = {
	{
	 ...
	 {
	 .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	 .name = "hdmi_hrt_timestamp",
	 .info = mt8695_afe_hdmi_timestamp_info,
	 .get = mt8695_afe_hdmi_timestamp_get,
...
static snd_pcm_uframes_t mt8695_afe_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	hw_ptr -= memif->phys_buf_addr;
	if (memif->avsync_mode) {
		hw_bef_read_index = memif->buf_read % memif->buffer_size;
