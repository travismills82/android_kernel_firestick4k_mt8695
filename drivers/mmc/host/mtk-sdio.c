...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	dev_err(host->dev, "sdio sclk: %d, timing: %d hz:%d cfg:0x%x\n", host->sclk, timing, hz,
				...
				struct mmc_command *cmd, struct mmc_data *data)
{
	...
	if (host->error == REQ_DAT_ERR) {
		dev_err(host->dev, "**********SDIO CRC ERROR************\n");
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	sdr_set_bits(host->base + MSDC_PAD_TUNE0,
...
#define AUTOK_CMD_LATCH_EN_NON_HS400_VALUE      (0)
#define AUTOK_CRC_LATCH_EN_HS400_VALUE          (0)
#define AUTOK_CRC_LATCH_EN_NON_HS400_VALUE      (0)
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	ret = irq_set_affinity(host->irq, cpumask_of(1)); /* move sdio interrupt to cpu1 */
	dev_err(host->dev, "Moving sdio irq to cpu1 %s\n", ret ? "failed" : "succeeded");
