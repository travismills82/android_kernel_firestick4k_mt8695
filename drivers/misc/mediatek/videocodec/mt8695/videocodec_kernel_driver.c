...
static void vcodec_setfastclk(void)
{
	clk_set_rate(clk_vdec_pll, 624000000);	/* Set vdecpll clk:624M */
}

static void vcodec_setslowclk(void)
{
	clk_set_rate(clk_vdec_pll, 324000000);	/* Set vdecpll clk:324M */
}
...
*/
			...
			if (grVcodecDecHWLock.pvHandle == 0) {	/* No one holds dec hw lock now */
				...
				if (rHWLock.eDriverType == VAL_DRIVER_TYPE_VP8_DEC ||
					rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC) {
					vcodec_setslowclk();
				}
...
static int vcodec_probe(struct platform_device *pdev)
{
	...
	clk_set_parent(clk_vdec_slow_sel, clk_vdec_pll);
