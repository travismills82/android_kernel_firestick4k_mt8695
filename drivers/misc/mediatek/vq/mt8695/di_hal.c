...
static int di_vdo_config_base(struct DI_VDO_PARAM_T *prParam)
{
	...
	DI_VDO_W_PICHEIGHT(prParam->height);	/* check */
	DI_VDO_W_DW_NEED(prParam->width / 4);	/* check */
	DI_VDO_W_HBLOCK_7_0(prParam->width / 8);	/* check */
	...
	DI_VDO_W_DW_NEED_HD((prParam->width / 4));	/* check */
#else
...
{
	unsigned int addry[5];	/*wxyza addr */
	unsigned int addrc[5];
	int idx = 0;
...
int di_vdo_config_set_param(struct DI_VDO_PARAM_T *prParam)
{
	...
	di_vdo_config_srcsize(prParam->width, prParam->height, prParam->src_fmt);

	di_vdo_config_hwsram(prParam->di_mode, prParam->width);

#if DI_VDO_SUPPORT_FUSION
	di_vdo1_config_startline(prParam->di_mode, prParam->width, VDO_VERTICAL_FILTER_4TAP,
				 prParam->src_fmt);
#else
...
int di_hal_config(struct mtk_vq_config_info *config_info)
{
	...
	if ((config->src_width <= TIMING_SIZE_720_480_H) &&
	    (config->src_height <= TIMING_SIZE_720_480_V)) {
		timing_type = VQ_TIMING_TYPE_480P;
	} else if ((config->src_width <= TIMING_SIZE_720_576_H) &&
		   (config->src_height <= TIMING_SIZE_720_576_V)) {
		timing_type = VQ_TIMING_TYPE_576P;
	} else if ((config->src_width <= TIMING_SIZE_1280_720_H) &&
		   (config->src_height <= TIMING_SIZE_1280_720_V)) {
		timing_type = VQ_TIMING_TYPE_720P;
	} else if ((config->src_width <= TIMING_SIZE_1920_1080_H) &&
		   (config->src_height <= TIMING_SIZE_1920_1080_V)) {
	...
	rDispfmtParam.width = config->src_width;
	rDispfmtParam.height = config->src_height;
	...
	rWcParam.width = config->src_width;
	rWcParam.height = config->src_height;
