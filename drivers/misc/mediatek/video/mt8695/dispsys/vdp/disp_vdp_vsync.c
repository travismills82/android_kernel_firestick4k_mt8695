	...
	struct vdp_hal_config_info *hal_config)
{
	...
	if ((buf1->src_fmt == DISP_HW_COLOR_FORMAT_YUV420_BLOCK) ||
		...
		DISP_LOG_E("VDP -- invalid color format\n");
		hal_config->cur_fb_info.is_scan_line = true;
	}
	...
	"cfg hal pts %d fd %d %p 0x%X (0x%X 0x%X 0x%X 0x%X)\n",
	buf1->pts,
	buf1->ion_fd,
	buf1->ion_hnd,
	buf1->src_phy_addr,
	...
	struct video_buffer_info *buf, int dsd_en)
{
	...
	if (dolby_path_start && !netflix_dolby_path_enable) {
		if ((buf->is_dolby || buf->is_dolby_vs10 || dovi_idk_dump || dovi_vs10_force)
			...
			DISP_LOG_N("set dobly path enable, pts %d\n", buf->pts);
...
#endif
	static unsigned int pre_pts[2];
	static int display_vsync_count = 0;
	static bool sub_path_stop = false;
		...
		for (i = thread_id; i < (thread_id + video_stream_number); i++) {
			...
			if (dolby_path_disable && !netflix_dolby_path_enable
				...
				if ((g_force_dolby > 0) && (dovi_vs10_path_enable_cnt == 2)) {
					disp_dovi_isr();
					DISP_LOG_E("dobly path enable process isr!\n");
					g_force_dolby = 0;
				}
				if (dovi_vs10_path_enable_cnt == 3) {
					fmt_hal_mix_plane(FMT_HW_PLANE_3);
					vdp_update_dovi_path_delay(false);
					DISP_LOG_E("dobly path enable mix graphic!\n");
				}
			}
...
#ifdef CONFIG_MTK_CLI_DEBUG_SUPPORT
			if (vdp_cli_get()->debug) {
				static char buffer[100];
				vdp_cli_get()->debug = 0;
				snprintf(buffer, sizeof(buffer), "get pts:%d\n", buf->pts);
				mdp_print(buffer);
			}
#endif

			if (i == 0) {
				static char displayBuffer[200] = {0};

				snprintf(displayBuffer, sizeof(displayBuffer), "buf pts:%d: source_duration:%d: vsync_duration:%d: remain:%d: display_count:%d\n",
					buf->pts,
					buf->source_duration,
					layer_info->vsync_duration,
					layer_info->display_duration,
					display_vsync_count);
				mdp_print(displayBuffer);
				display_vsync_count = 0;
			}
			...
			if (layer_info->timeline_idx > layer_info->release_idx) {
				if ((layer_info->timeline_idx - layer_info->release_idx) > duration) {
					...
					if (layer_info->state == VDP_LAYER_STOPPED) {
						...
						DISP_LOG_N("releasing1...tl_idx=%d, rel_idx=%d, tl_value=%d\n",
							layer_info->timeline_idx,
							layer_info->release_idx,
							layer_info->timeline->value);
						continue;
					...
					DISP_LOG_N("releasing2...tl_idx=%d, rel_idx=%d, tl_value=%d\n",
								layer_info->timeline_idx,
			...
			*DISP_LOG_N("timeline_idx increase %d, buf fence_index=%d\n",
			...
			if (vdp_cli_get()->enable_pts_debug) {
				...
				DISP_LOG_E("routine: layer %d, pre_pts %d, count %d, duration %d, pts %d, timer %ld\n",
