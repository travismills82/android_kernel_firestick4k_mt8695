...
UINT32 dovi_set_video_input_format(enum dovi_signal_format_t e_input_format)
{
	if (p_cp_param != NULL) {
		...
		dovi_default("[DOVI]Re-Update HDR Type Input:%d\n", e_input_format);
	}
...
{
	dovi_func();
	dovi_default("set_graphic_format %d\n", g_format);
...
{
	dovi_func();
	dovi_default("update_graphic_info\n");
...
{
	dovi_func();
	dovi_default("get output format %d\n", p_cp_param->output_format);
		...
		if (hdr_metadata->dr_range == DISP_DR_TYPE_DOVI) {
			...
			if (dovi_md_parser_enable == false) {
				...
				*p_frame_num = 0;
				...
				first_frame = 1;
				*p_frame_num = 0;
				dovi_error("seamless change dovi input again\n");
			}
