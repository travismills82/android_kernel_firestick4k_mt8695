...
int32_t fmt_hal_set_delay(uint32_t fmt_id, uint32_t adj_f, uint32_t h_delay, uint32_t v_delay)
{
	...
	FMT_FUNC();
	FMT_LOG_I("fmt_id: %d, h_delay: %d, v_delay: %d\n", fmt_id, h_delay, v_delay);
...
int32_t fmt_hal_set_pixel_factor(uint32_t fmt_id, uint32_t pixel_len, uint32_t hsfactor, uint32_t enable)
{
	struct disp_fmt_t *disp_fmt = NULL;
	...
	disp_fmt->sw_fmt_base->rField.fgHSLR = (enable && (hsfactor == 0x1000));
